        ...
        a.Suggest = Class.create({
            ...
            initialize: function (c, d) {
                    ...
                    for (var h = 0; h < this.sources.length; h++) {
                        ...
                        var m = new Ajax.Request(
                            e, {
                                ...
                                onSuccess: this.setSuggestions.bindAsEventListener(this, d, f),
                ...
                getSuggestionList: function (d, u) {
                    ...
                    for (var z = 0; z < t.length; z++) {
                        ...
                        for (var l in this.options.resultInfo) {
                            ...
                            if (g.extern) {
                                ...
                                A.insert({
                                    bottom: new Element("dt", {
                                        "class": w + " " + sectionClass
                                    }).insert({
                                        bottom: p
                                    })
                                });
                            ...
                            n(t[z], y).each(
                                function (E) {
                                    ...
                                    s.insert({
                                        bottom: new Element("div").update(F)
                                    })
                                }
                            )
                        }
                        ...
                        if (this.options.resultCategory) {
                            var v = new Element("span", {
                                "class": "hidden term-category"
                            });
                            n(t[z], this.options.resultCategory).each(function (k) {
                                v.insert(new Element("input", {
                                    type: "hidden",
                                    value: k
                                }))
                            })
                        ...
                        B.push({
                            id: e(t[z], u.resultId || this.options.resultId),
                            value: e(t[z], u.resultValue || this.options.resultValue),
                            altName: q,
                            info: A,
                            category: v
                        })
                    ...
                    var k = this.container.fire("ms:suggest:containerPrepared", {
                        container: this.container,
                        suggest: this
                    });
...
//                	debugger
                    ...
                    if (f.length == 0) {
                        h.addItem(new b.widgets.XListItem(this.options.noresults, {
                    ...
                    if (this.fld.hasClassName("accept-value")) {
                        ...
                        h.addItem(this.generateListItem({
                            id: this.fld.value,
                            value: this.fld.value,
                            category: d,
                            info: new Element("div", {
                                "class": "hint"
                            }).update("(你输入的内容不是一个标准的术语)")
                        }, "custom-value", true))
                    }
                    return h.getElement()
                }, generateListItem: function (k, d, l) {
                    var f = new Element("div", {
                        "class": "tooltip-" + this.options.tooltip
                    });
                    if (k.icon) {
                        f.insert(new Element("img", {
                            src: k.icon,
                            "class": "icon"
                        }))
                    }
                    ...
                    if (this.options.tooltip && !l) {
                        var e = new Element("span", {
                            "class": "fa fa-info-circle xHelpButton " + this.options.tooltip,
                            title: k.id
                        });
                    ...
                    if (k.altName) {
                        h.insert({
                            top: new Element("span", {
                                "class": "matching-alternative-name"
                            }).update(k.altName)
                        })
                    }
                    var c = new Element("div").insert(new Element("span", {
                        "class": "suggestId"
                    }).update(k.id)).insert(new Element("span", {
                        "class": "suggestValue"
                    }).update(k.value)).insert(new Element("div", {
                        "class": "suggestCategory"
                    }).update(k.category));
                    var g = new b.widgets.XListItem(f, {
                        containerClasses: "suggestItem " + (d || ""),
                        value: c,
                        noHighlight: true
                    });
                    Event.fire(this.fld, "ms:suggest:suggestionCreated", {
                    ...
                    if (c && c.parentNode) {
                        ...
                        document.fire("ms:suggest:clearSuggestions", {
                            suggest: this
                        })
                    }
                }
...
//            debugger
            ...
            this.onItemAdded(h);
            return h
        }, onItemAdded: function (c) {}, removeItem: function (d) {
            var c = d.findElement("li");
            this.synchronizeSelection({
                value: (c.down("input[type=checkbox]") || c.down("input")).value,
                checked: false
            });
            c.remove();
            ...
            if (g) {
                if (this.input.hasClassName("generateYesNo") && !f.up(".yes-no-picker")) {
                    ...
                    var d = YesNoPicker.generatePickerElement([{
                        type: "na",
                        selected: !isValueSelected(e, h) && !isValueSelected(c, h)
                    }, {
                        type: "yes",
                        name: e,
                        selected: isValueSelected(e, h)
                    }, {
                        type: "no",
                        name: c,
                        selected: isValueSelected(c, h)
                    }], h, k, true, f.next());
                    f.insert({
                        before: d
                    });
    ...
    a.ModalPopup = Class.create({
        ...
        initialize: function (e, c, d) {
            ...
            this.dialog = new Element("div", {
                "class": "msdialog-modal-container"
            });
            ...
            case "top":
                this.dialogBox.setStyle({
                    top: (document.viewport.getScrollOffsets().top + 6) + "px"
                });
                break;
            case "bottom":
                this.dialogBox.setStyle({
            ...
            this.dialogBox.setStyle({
                left: (document.viewport.getScrollOffsets().left + d) + "px",
                top: (document.viewport.getScrollOffsets().top + c) + "px",
                margin: "0"
            })
            ...
            for (var d = 0; d < c.size(); ++d) {
                if (Prototype.Browser.IE || Prototype.Browser.WebKit) {
                    shortcut.add(c[d], f.bindAsEventListener(this, e), {
                        type: "keyup"
                    })
                } else {
                    shortcut.add(c[d], f.bindAsEventListener(this, e), {
...
var PhenoTips = (function (b) {
    var a = b.widgets = b.widgets || {};
    a.SolrQueryProcessor = Class.create({
...
var PhenoTips = (function (b) {
    var a = b.widgets = b.widgets || {};
    a.OntologyBrowser = Class.create({
        options: {
            script: "/bin/get/PhenoTips/SolrService?sort=nameSort asc&start=0&rows=10000&",
            varname: "q",
            method: "post",
            json: true,
            ...
            targetQueryProcessor: typeof (b.widgets.SolrQueryProcessor) == "undefined" ? null : new b.widgets.SolrQueryProcessor({
                id: {
                    activationRegex: /^HP:[0-9]+$/i,
                    mandatory: true,
            ...
            expandQueryProcessor: typeof (b.widgets.SolrQueryProcessor) == "undefined" ? null : new b.widgets.SolrQueryProcessor({
                is_a: {
                    activationRegex: /^HP:[0-9]+$/i,
                    mandatory: true,
                    transform: function (c) {
            ...
            enableBrowse: true,
            isTermSelected: function (c) {
                return false
            }, unselectTerm: function (c) {}, defaultEntryAction: "browse"
        },
        initialize: function (e, c, d) {
            this.options = Object.extend(Object.clone(this.options), d || {});
            this.suggest = e;
            this.loadingMessage = new Element("div", {
                "class": "plainmessage loading"
            }).update("Loading...");
            if (c) {
                this.container = c
            } else {
                this.container = new b.widgets.ModalPopup(this.loadingMessage, {}, {
            ...
            var c = new Ajax.Request(d, {
                ...
                onSuccess: function (k) {
                    ...
                    this.__crtRoot = h;
                    Event.fire(document, "xwiki:dom:updated", {
                        elements: [this.container.content]
                    })
                }.bind(this),
            ...
            var c = new Ajax.Request(e, {
                ...
                onSuccess: function (n) {
                    ...
                    if (d) {
                        ...
                        Event.fire(document, "obrowser:content:added", {
                            added: l,
                            obrowser: this
                        });
                    ...
                    } if ((m.count === 0) || (d && !f.hasClassName("root") && !f.down(".descendents .entry, .error"))) {
                    ...
                    Event.fire(document, "ms:popup:content-updated", {
                        popup: this.container
                    })
                }.bind(this),
                onFailure: function (l) {
                    Event.fire(f, "obrowser:expand:failed", {
                        data: new Element("div", {
                            "class": "error"
                        }).update("Failed to retrieve data : " + respose.statusText),
                        count: -1
                    })
                }, onComplete: function () {
            ...
            if (this.options.showParents) {
                ...
                this._getResultFieldValueAsArray(c, this.options.resultParent.selector).each(function (k) {
                    ...
                    f.insert({
                        bottom: this._createParentBranch(l)
                    })
                }.bind(this));
                if (f.hasChildNodes()) {
                    e.insert({
            ...
            var h = {
                id: this._getResultFieldValue(c, this.options.resultId),
                value: this._getResultFieldValue(c, this.options.resultValue),
                category: this._generateEntryCategory(c)
            };
            ...
            for (var d = 0; d < e.length; d++) {
                var f = {
                    id: this._getResultFieldValue(e[d], this.options.resultId),
                    value: this._getResultFieldValue(e[d], this.options.resultValue),
                    category: this._generateEntryCategory(e[d])
                };
                g.insert({
                    bottom: this._createDescendentBranch(f)
                })
            }
            if (g.hasChildNodes()) {
                return g
            ...
            return new Element("div", {
                "class": "descendents hint empty"
            }).update(this.options.noresults)
        }, _createBranch: function (h, e, g, c) {
            var d = new Element(h, {
                "class": "entry " + e
            });
            d.__termId = g.id;
            d.__termCategory = g.category;
            var l = new Element("div", {
                "class": "entry-data"
            });
            l.insert({
                bottom: this._generateEntryTitle(g.id, g.value)
            });
            var f = new Element("span", {
                "class": "entry-tools"
            });
            f.observe("click", function (m) {
                m.stop()
            });
            l.insert({
                bottom: f
            ...
            d.update(l);
            if (!this._isRootEntry(d)) {
                if (this.options.defaultEntryAction == "browse") {
                    l.down(".info").observe("click", this._browseEntry.bindAsEventListener(this))
                }
            }
            f.insert(new Element("span", {
                "class": "fa fa-info-circle phenotype-info xHelpButton",
                title: g.id
            }));
            if (this.options.enableSelection) {
                d.__selectTool = new Element("input", {
                    type: "checkbox",
                    name: "term_selector",
                    value: g.id,
                    "class": "select-tool"
                ...
                if (this.options.isTermSelected(d.__termId)) {
                    d.addClassName("accepted");
                    d.__selectTool.checked = "checked"
                }
                d.__selectTool.observe("click", this._toggleEntrySelection.bindAsEventListener(this));
                if (this.options.defaultEntryAction == "select") {
                    l.down(".info").observe("click", this._toggleEntrySelection.bindAsEventListener(this))
                }
            }
            if (c) {
                var k = new Element("span", {
                    "class": "expand-tool"
                }).update(this._getExpandCollapseSymbol(!d.hasClassName("root")));
                k.observe("click", function (n) {
                    var m = n.element().up(".entry");
                    if (!this._isExpandToolLocked(m)) {
                        this._toggleExpandState(m)
                    }
                }.bindAsEventListener(this));
                l.insert({
                    top: k
                });
                this.expand(d, d.hasClassName("root"))
            }
            return d
            ...
            if (c) {
                ...
                c.down(".expand-tool").update(this._getExpandCollapseSymbol(c.hasClassName("collapsed")))
            }
            ...
            if (this.suggest) {
                ...
                this.suggest.acceptEntry({
                    id: c.__termId,
                    value: d,
                    category: c.__termCategory,
                    negative: c.down(".selected.no")
                }, d, "", true);
            ...
            this.container.setContent(new Element("div", {
                "class": "ontology-tree"
            }).update(c))
        }, show: function (c) {
            if (c) {
...
StickyBox = Class.create({
    ...
    initialize: function (b, a, c) {
        ...
        if (this.options.resize) {
            var b = {
                diff: (c - this.boxHeight),
                original: this.boxHeight
            };
            this.boxHeight = c;
...
var PhenoTips = (function (b) {
    var a = b.widgets = b.widgets || {};
    a.FreeMultiselect = Class.create({
        counter: 1,
        options: {
        ...
        initialize: function (f, e) {
            ...
            if (f.down("input.xwiki-free-multiselect-value")) {
                ...
                f.insert(g.wrap("li"));
                g.observe("click", function (l) {
                    l.stop();
                    var m = g.up("li").previous();
                    var k = m && m.down("input.xwiki-free-multiselect-value");
                    if (k) {
                        h.generateInput(k)
                    }
                }.bindAsEventListener(this))
            ...
            var c = new Element("input", {
                ...
                type: d.type,
                size: d.size,
                "class": "xwiki-free-multiselect-value"
            });
            var e = new Element("li");
            e.insert(c);
            d.up(".xwiki-free-multiselect-line").insert({
                after: e
            });
            this.enhanceLine(c);
            c.focus()
...
document.observe("xwiki:dom:loaded", function () {
    $$(".xwiki-free-multiselect").each(function (a) {
        new PhenoTips.widgets.FreeMultiselect(a)
    })
});
var XWiki = (function (c) {
    var a = c.widgets = c.widgets || {};
    a.VisibilityController = Class.create({
        initialize: function (d) {
            this.element = d;
            this.controller = this.element.down(".controller input[type=checkbox]");
            if (!this.controller) {
                return
            }
            ...
            this.controller.observe("change", this.controlVisibility.bindAsEventListener(this))
...
var PhenoTips = (function (b) {
    ...
    a.UnitConverter = Class.create({
        ...
        initialize: function (e, c, f, d, g) {
            ...
            this._trigger = new Element("select", {
                "class": "unit-system-selector"
            });
            ...
            var f = new Element("div", {
                "class": "unit-type imperial"
            });
            h.insert(g).insert(f);
            h._meta.imperial_units.each(function (m) {
                f.insert(new Element("label").insert(new Element("input", {
                    style: "width: auto",
                    name: m,
                    type: "text",
                    size: 3,
...
var PhenoTips = (function (c) {
    var a = c.widgets = c.widgets || {};
    a.HelpButton = Class.create({
        infoServices: {
            xHelpButton: {
                hint: "How to enter this information",
                callback: function (d) {
                    d.helpBox.content.update(d._information || "")
                }
            },
            "phenotype-info": {
                hint: "About this phenotype",
                service: "/bin/get/PhenoTips/PhenotypeInfoService",
                callback: function (k, h) {
                    var n = k.helpBox.content;
                    var g = function (o, l, p) {
                        return new Element(o, l && {
                            "class": l
                        } || {}).update(p || "")
                    };
                    ...
                    if (c.widgets.OntologyBrowser) {
                        ...
                        n.insert(g("div", "term-tools").insert(f.wrap("span", {
                            "class": "buttonwrapper"
                        })))
                    }
                }
            ...
            "omim-disease-info": {
                ...
                service: "/bin/get/PhenoTips/OmimInfoService",
                callback: function (k, g) {
                    var n = k.helpBox.content;
                    var f = function (o, l, p) {
                        return new Element(o, l && {
                            "class": l
                        } || {}).update(p || "")
                    };
                    n.update(f("span", "info").insert(f("span", "key").update(g.id)).insert(" ").insert(f("span", "value").update(g.label.splice(0, 1)[0])));
                    var m = {
                        label: "",
                        symptoms: "This disorder is typically characterized by",
                        not_symptoms: "This disorder does not typically cause"
                    };
                    var d = f("dl");
                    for (var e in m) {
                        if (g[e] && g[e].length > 0) {
                    ...
                    var h = new Element("a", {
                        "class": "button",
                        href: "http://www.omim.org/entry/" + g.id,
                        target: "_blank"
                    }).update("Read about it on OMIM.org...");
                    n.insert(f("div", "term-tools").insert(h.wrap("span", {
                        "class": "buttonwrapper"
                    })));
            ...
            "gene-info": {
                hint: "About this gene",
                service: "/bin/get/PhenoTips/GeneInfoService",
                callback: function (f, n) {
                    var k = f.helpBox.content;
                    var e = function (p, l, q) {
                        return new Element(p, l && {
                            "class": l
                        } || {}).update(q || "")
                    };
                    ...
                    if (n.external_ids) {
                        ...
                        var m = [{
                            ...
                            name: "Ensembl",
                            url: "http://useast.ensembl.org/Homo_sapiens/Gene/Compara_Tree?g=",
                        ...
                        m.each(function (l) {
                            ...
                            if (p) {
                                ...
                                p.each(function (q) {
                                    h.insert(new Element("a", {
                                        href: l.url + q,
                                        "class": "button"
                                    }).update(l.name + ": " + q).wrap("span", {
                                        "class": "buttonwrapper"
                                    }))
                                })
                            }
        ...
        initialize: function (e) {
            ...
            var d = this;
            new Ajax.Request(d._builder.service, {
                parameters: {
                    id: d._information
                },
                onCreate: function () {
                    d.helpBox.content.update(new Element("span", {
                        "class": "hint temporary"
                    }).update("Loading..."))
                    ...
                    d.helpBox.insert("Failed to retrieve information about " + d._information + " : " + e.statusText)
                }
            })
        }
    });
